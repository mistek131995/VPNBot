...
namespace VpnBotApi.Worker.TelegramBot.Common
{
    public class TelegramBotWebClient(IConfiguration configuration)
    {
        ...
        private async Task<Guid> AddUserToInbound(long telegramUserId, DateTime endDate)
        {
            ...
            var newVpnUser = GenerateNewVpnUser(telegramUserId, endDate);
        ...
        /// <summary>
        /// Создает пользователя для добавления в подключение
        /// </summary>
        /// <param name="telegramUserId"></param>
        /// <param name="endDate"></param>
        /// <returns></returns>
        private InboundUser GenerateNewVpnUser(long telegramUserId, DateTime endDate)
        {
            var expiryTime = endDate.Subtract(new DateTime(1970, 1, 1)).TotalMilliseconds;

            var user = new InboundUser
            {
                Id = Guid.NewGuid(),
                Flow = "",
