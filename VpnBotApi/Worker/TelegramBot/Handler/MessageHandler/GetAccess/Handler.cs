    ...
    public class Handler(IRepositoryProvider provider, TelegramBotWebClient webClient) : IHandler<Query, Response>
    {
        ...
        public async Task<Response> HandlingAsync(Query query)
        {
            ...
            if (access == null)
            {
                ...
                response.Text = $"Получен тестовый доступ до {user.Access.EndDate.ToString("d")}. Скачайте приложение и отсканируйте QR код. " +
            ...
            {
                response.Text = $"Доступ активен до {access.EndDate.ToString("d")}. Скачайте приложение и отсканируйте QR код.";
